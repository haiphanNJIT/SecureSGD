...
def TaylorExp(logits, labels, adv_logits, b_labels, L, alpha, perturbW):
    ...
    Taylor_benign = math_ops.add(relu_logits - logits * labels, math.log(2.0) + 0.5*neg_abs_logits + 1.0/8.0*neg_abs_logits**2)
    
    Taylor_adv = tf.abs(adv_logits - b_labels)'''
    
    ### Adversarial training loss
    #adv_loss = (1/(L + alpha*L))*(Taylor_benign + alpha * Taylor_adv)
    adv_loss = (1/(1 + alpha))*(Taylor_benign + alpha * Taylor_adv)
    #adv_loss = (alpha * Taylor_benign + Taylor_adv)
...
def TaylorExpAdLM(logits, labels, perturbW):
    ...
    cross_entropy_mean = tf.reduce_mean(Taylor_benign, name='cross_entropy') + tf.reduce_sum(perturbW, name = 'perturbW');
...
def train(total_loss, global_step, lr, _var_list):
  ...
  with tf.control_dependencies([loss_averages_op]):
    ...
    grads_and_vars = opt.compute_gradients(total_loss, var_list=_var_list)
  ...
  variables_averages_op = variable_averages.apply(_var_list)
