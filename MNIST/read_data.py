...
def read_max():
    ...
    all_lines = []
    for i in range(2):
        with open('DPSGD_max_{}.txt'.format(i+1), 'r') as inf:
...
def find_max():
    ...
    with open('DPSGD_run_{}.txt'.format(data_file_n), 'r') as inf:
    ...
    for line in all_lines:
        if line.strip() == 'new_line':
            if i >= 0:
                max_benign = max(benigns[i])
                max_ifgsm = max(ifgsms[i])
                max_fgsm = max(fgsms[i])
                max_mim = max(mims[i])
                max_madry = max(madrys[i])
        ...
        if line[0] == 's':
            data = line.split()
            try:
                benigns[i].append(float(data[benign_offset]))
                ifgsms[i].append(float(data[ifgsm_offset]))
                fgsms[i].append(float(data[fgsm_offset]))
                mims[i].append(float(data[mim_offset]))
                madrys[i].append(float(data[madry_offset]))
            except:
                print(line)
    max_benign = max(benigns[i])
    max_ifgsm = max(ifgsms[i])
